...
void Rope::Solve(float deltaMs)
{
    ...
    for (int iter = 0; iter < stickIterations_; ++iter)
    {
        for (auto &c : constraints_)
        {
            ...
            if (!c.a->pinned) c.a->pos +=  dir * wA * correctionMag;
            if (!c.b->pinned) c.b->pos += dir.Negate() * wB * correctionMag;
